...
const YearlyQues = () => {
  ...
  const listOfLocalIssues = ['0'];
  const listOfNationalIssues = ['0'];
  ...
  const submitHandler = (e) => {
    ...
    const data = {
      ...
      Impact_on_voting: {
        ...
        is_candidate_known_for_years: q7bRef.current?.value,
        // I have personal connect with the candidate ->d
        is_candidate_from_my_area: q7cRef.current?.value,
        is_candidate_help_us_solve_problem: q7eRef.current?.value,
        is_candidate_popular: q7fRef.current?.value,
        is_candidate_with_strong_ideology: q7gRef.current?.value,
        is_candidate_nonCorrupt_or_honest: q7hRef.current?.value,
        will_analyse_past_of_the_candidate: q7iRef.current?.value,
        based_on_suggestion_of_my_problem: q7jRef.current?.value,
        based_on_suggestion_of_my_community: q7kRef.current?.value,
        is_candidate_active_in_my_area: q7lRef.current?.value,
        is_candidate_available_in_my_area: q7mRef.current?.value,
      },
      Parameters_to_vote: {
        ...
        wont_hesitate_to_vote_any_new_party_or_candidate_if_deserving: parseInt(
  ...
  return (
    <>
      <form style={{ margin: '10px 10px' }} onSubmit={submitHandler}>
        <div
